...
Vector Transformation::vector_multiply(Matrix m, Vector v) {
	...
	result.x = m.items[0][0]*v.x + m.items[1][0]*v.y + m.items[2][0]*v.z + m.items[3][0]*v.w;
	result.y = m.items[0][1]*v.x + m.items[1][1]*v.y + m.items[2][1]*v.z + m.items[3][1]*v.w;
	result.z = m.items[0][2]*v.x + m.items[1][2]*v.y + m.items[2][2]*v.z + m.items[3][2]*v.w;
	result.w = m.items[0][3]*v.x + m.items[1][3]*v.y + m.items[2][3]*v.z + m.items[3][3]*v.w;
...
Matrix Transformation::inverse(Matrix m) {
	...
	if (det != 0){
		...
		cout << "B Matrix" << endl;
		cout << "b11 = " << b11 << ", D = " << D << ", b11/D = " << b11*(D*-1) << endl;
		result.print();
	} else {
